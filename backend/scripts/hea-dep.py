...
def create_deployment():
    ...
    metadata = client.V1ObjectMeta(name="healthymarriagedb-deployment")

    # Define the selector for the Deployment
    selector = client.V1LabelSelector(match_labels={"app": "healthymarriagedb"})

    # Define the labels for the Deployment's template
    labels = {"app": "healthymarriagedb"}

    # Define the container spec for the Deployment
    container = client.V1Container(
        name="healthymarriagedb-container",
        image="hub.leandevclan.com/healthymarriagedb:latest",
